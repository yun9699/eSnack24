  ...
  <div class="max-w-2xl mx-auto p-6 bg-gray-50 rounded-lg shadow-lg">
    ...
    <div v-else>
      <div v-if="requestProduct" class="space-y-4">
        <div>
          <h2 class="text-lg font-semibold text-gray-700">리스트 번호: {{ requestProduct.cpno }}</h2>
        </div>
        <div>
          <p class="text-gray-700">제목: {{ requestProduct.cptitle }}</p>
        </div>
        <div>
          <p class="text-gray-700">내용: {{ requestProduct.cpproduct }}</p>
        </div>
        <div>
          <p class="text-gray-700">답변: {{ requestProduct.cpanswer }}</p>
        </div>

        <div class="text-xs text-gray-500">
          <p>등록일: {{ formatDate(requestProduct.cpregdate) }}</p>
          <p>수정일: {{ formatDate(requestProduct.cpmoddate) }}</p>
        </div>
      </div>
