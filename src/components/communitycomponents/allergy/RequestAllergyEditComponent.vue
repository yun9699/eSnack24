  ...
  <div class="max-w-2xl mx-auto p-6 bg-gray-50 rounded-lg shadow-lg">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-2xl font-bold text-gray-800">
        {{ t('allergyReportEdit.allergyReportEditHeader') }}
      </h1>
    </div>

    <div v-if="loading" class="text-center text-gray-500">
      {{ t('allergyReportEdit.loadingAllergyReportEdit') }}
    </div>
    <div v-else>
      <div v-if="requestAllergy" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700">
            {{ t('allergyReportEdit.allergyReportEditLabels.title') }}
          </label>
          <textarea
              v-model="catitle"
              rows="4"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700">
            {{ t('allergyReportEdit.allergyReportEditLabels.content') }}
          </label>
          <textarea
              v-model="caallergy"
              rows="4"
        ...
        >
          {{ submitting
            ? t('allergyReportEdit.allergyReportEditButtons.edit.loading')
            : t('allergyReportEdit.allergyReportEditButtons.edit.default')
          }}
        </button>
        ...
        <div class="mt-4 text-right">
          <button
              ...
              :disabled="submitting"
          >
            {{ t('allergyReportEdit.allergyReportEditButtons.delete') }}
          </button>
        </div>
      </div>

      <div v-else class="text-center text-gray-400 py-6">
        {{ t('allergyReportEdit.noAllergyReportData') }}
      </div>
    </div>
  </div>
</template>